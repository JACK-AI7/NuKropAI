...
class RecommendationService {
    ...
    static getRecommendation(label, lat, lng, weather) {
    ...
    static findMatchingDiseaseKey(plantName, diseaseName) {
        const data = this.getData();
        const pNorm = normalizeKey(plantName || '');
        const dNorm = normalizeKey(diseaseName || '');
        if (!dNorm)
            return null;
        const tryCrop = (cropKey) => {
            ...
            for (const diseaseKey of Object.keys(crop.diseases)) {
                ...
                if ((dNorm && nameNorm && (nameNorm.includes(dNorm) || dNorm.includes(nameNorm))) ||
    ...
    static getSoilInsights(type, lat, lng) {
        const data = this.getData();
